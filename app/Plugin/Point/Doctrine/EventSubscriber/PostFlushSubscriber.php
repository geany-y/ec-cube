...
class PostFlushSubscriber implements EventSubscriber
{
    ...
    {
        ezd('postFlush');
        //ezd(get_class_methods($args));
        $date = new \DateTime("now");
        $findProductFlg = false;
        $uow = $args->getEntityManager()->getUnitOfWork();
        ezd(count($uow->getIdentityMap()));
        foreach ($uow->getIdentityMap() as $key => $updated) {
            if($key == 'Eccube\Entity\Product'){
                ezd('OK!!!!!');
                echo '<pre>';
                var_dump(array_values($updated)[0]->getId());
                echo '</pre>';
                exit();
            }
